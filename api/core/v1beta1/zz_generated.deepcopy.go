...
func (in *FeatureFlagSourceSpec) DeepCopyInto(out *FeatureFlagSourceSpec) {
	*out = *in
	...
	if in.ContextValues != nil {
		in, out := &in.ContextValues, &out.ContextValues
		*out = make(map[string]string, len(*in))
		for key, val := range *in {
			(*out)[key] = val
		}
	}
	if in.HeaderToContextMappings != nil {
		in, out := &in.HeaderToContextMappings, &out.HeaderToContextMappings
		*out = make(map[string]string, len(*in))
		for key, val := range *in {
			(*out)[key] = val
		}
