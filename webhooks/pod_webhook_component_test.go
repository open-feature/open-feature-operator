...
const (
	...
	flagSourceConfigurationNamespace = "test-validate-featureflagconfiguration"
	featureFlagSpec                  = `
	{
      "flags": {
        "new-welcome-message": {
...
// 			fmt.Sprintf("%s/%s", OpenFeatureAnnotationPrefix, FlagSourceConfigurationAnnotation): fmt.Sprintf("%s/%s", mutatePodNamespace, flagSourceConfigurationName),
// 		})
...
// 		err := k8sClient.Get(
// 			testCtx, client.ObjectKey{Name: flagSourceConfigurationName, Namespace: mutatePodNamespace}, ffConfig,
// 		)
// 		require.Nil(t, err)
...
// 			Name:      flagSourceConfigurationName,
// 			Namespace: mutatePodNamespace,
// 		}, cm)
// 		require.Nil(t, err)

// 		require.Equal(t, cm.Name, flagSourceConfigurationName)
// 		require.Equal(t, cm.Namespace, mutatePodNamespace)
// 		require.EqualValues(t, map[string]string{
// 			fmt.Sprintf("%s/%s", OpenFeatureAnnotationPrefix, FlagSourceConfigurationAnnotation): flagSourceConfigurationName,
// 		}, cm.Annotations)
// 		require.Equal(t, 2, len(cm.OwnerReferences))

// 		require.Equal(t, cm.Data, map[string]string{
// 			fmt.Sprintf("%s_%s.flagd.json", mutatePodNamespace, flagSourceConfigurationName): ffConfig.Spec.FeatureFlagSpec,
// 		})
...
// 		pod := testPod(defaultPodName, defaultPodServiceAccountName, map[string]string{
// 			OpenFeatureAnnotationPrefix: "enabled",
// 			fmt.Sprintf("%s/%s", OpenFeatureAnnotationPrefix, FlagSourceConfigurationAnnotation): fmt.Sprintf("%s/%s", mutatePodNamespace, ffConfigName),
// 		})
// 		err = k8sClient.Create(testCtx, pod)
// 		require.Nil(t, err)
...
// 	ns := &corev1.Namespace{}
// 	ns.Name = flagSourceConfigurationNamespace
...
// 	ffConfig := &v1alpha1.FeatureFlagConfiguration{}
// 	ffConfig.Namespace = mutatePodNamespace
// 	ffConfig.Name = flagSourceConfigurationName
...
// 	ffConfig2.Name = flagSourceConfigurationName2
...
// 		{
// 			Source: fmt.Sprintf("%s/%s", mutatePodNamespace, flagSourceConfigurationName),
// 		},
// 		{
// 			Source:   fmt.Sprintf("%s/%s", mutatePodNamespace, flagSourceConfigurationName2),
