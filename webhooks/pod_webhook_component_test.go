...
const (
	...
	featureFlagSourceName          = "test-flag-source-configuration"
	featureFlagSourceName2         = "test-flag-source-configuration-2"
	featureFlagSourceName3         = "test-flag-source-configuration-3"
	...
	featureFlagSourceNamespace     = "test-validate-featureflag"
	featureFlagSpec                = `
	{
      "flags": {
        "new-welcome-message": {
...
// 		flagConfig, _ := v1alpha1.NewFeatureFlagSourceSpec()
// 		pod := testPod(defaultPodName, defaultPodServiceAccountName, map[string]string{
// 			OpenFeatureAnnotationPrefix: "enabled",
// 			fmt.Sprintf("%s/%s", OpenFeatureAnnotationPrefix, FeatureFlagSourceAnnotation): fmt.Sprintf("%s/%s", mutatePodNamespace, featureFlagSourceName),
// 		})
...
// 	t.Run("should create flagd sidecar even if openfeature.dev/featureflag annotation isn't present", func(t *testing.T) {
// 		pod := testPod(defaultPodName, defaultPodServiceAccountName, map[string]string{
// 			OpenFeatureAnnotationPrefix: "enabled",
// 		})
...
// 		ffConfig := &v1alpha1.FeatureFlag{}
// 		err := k8sClient.Get(
// 			testCtx, client.ObjectKey{Name: featureFlagSourceName, Namespace: mutatePodNamespace}, ffConfig,
// 		)
// 		require.Nil(t, err)

// 		ffConfig.Spec = v1alpha1.FeatureFlagSpec{
...
// 		require.Equal(t, cm.Name, featureFlagSourceName)
// 		require.Equal(t, cm.Namespace, mutatePodNamespace)
// 		require.EqualValues(t, map[string]string{
// 			fmt.Sprintf("%s/%s", OpenFeatureAnnotationPrefix, FeatureFlagSourceAnnotation): featureFlagSourceName,
// 		}, cm.Annotations)
// 		require.Equal(t, 2, len(cm.OwnerReferences))

// 		require.Equal(t, cm.Data, map[string]string{
// 			fmt.Sprintf("%s_%s.flagd.json", mutatePodNamespace, featureFlagSourceName): ffConfig.Spec.FeatureFlagSpec,
// 		})
...
// 		pod := testPod(defaultPodName, defaultPodServiceAccountName, map[string]string{
// 			OpenFeatureAnnotationPrefix: "enabled",
// 			fmt.Sprintf("%s/%s", OpenFeatureAnnotationPrefix, FeatureFlagSourceAnnotation): fmt.Sprintf("%s/%s", mutatePodNamespace, ffConfigName),
// 		})
// 		err = k8sClient.Create(testCtx, pod)
// 		require.Nil(t, err)
...
// 	t.Run(`should only write non default featureflagsource env vars to the flagd container`, func(t *testing.T) {
// 		pod := testPod(defaultPodName, defaultPodServiceAccountName, map[string]string{
// 			OpenFeatureAnnotationPrefix: "enabled",
// 			fmt.Sprintf("%s/%s", OpenFeatureAnnotationPrefix, FeatureFlagSourceAnnotation): fmt.Sprintf("%s/%s", mutatePodNamespace, featureFlagSourceName),
// 			"openfeature.dev/featureflagsource":                                            fmt.Sprintf("%s/%s", mutatePodNamespace, featureFlagSourceName),
// 		})
// 		err := k8sClient.Create(testCtx, pod)
// 		require.Nil(t, err)
// 		defer podMutationWebhookCleanup(t)
...
// 	t.Run(`should use env var configuration to overwrite featureflagsource defaults`, func(t *testing.T) {
...
// 	t.Run(`should overwrite env var configuration with featureflagsource values`, func(t *testing.T) {
...
// 	t.Run("should create flagd sidecar using featureflagsource", func(t *testing.T) {
// 		t.Setenv(v1alpha1.SidecarEnvVarPrefix, "")
// 		t.Setenv(fmt.Sprintf("%s_%s", v1alpha1.InputConfigurationEnvVarPrefix, v1alpha1.SidecarMetricPortEnvVar), "")
// 		t.Setenv(fmt.Sprintf("%s_%s", v1alpha1.InputConfigurationEnvVarPrefix, v1alpha1.SidecarPortEnvVar), "")
// 		t.Setenv(fmt.Sprintf("%s_%s", v1alpha1.InputConfigurationEnvVarPrefix, v1alpha1.SidecarSocketPathEnvVar), "")
...
// 		pod := testPod(defaultPodName, defaultPodServiceAccountName, map[string]string{
// 			fmt.Sprintf("%s/%s", OpenFeatureAnnotationPrefix, EnabledAnnotation): "true",
// 			"openfeature.dev/featureflagsource":                            fmt.Sprintf("%s/%s", mutatePodNamespace, featureFlagSourceName2),
// 		})
// 		err := k8sClient.Create(testCtx, pod)
// 		require.Nil(t, err)
// 		defer podMutationWebhookCleanup(t)
...
// 	t.Run("should not create flagd sidecar if featureflagsource does not exist", func(t *testing.T) {
// 		pod := testPod(defaultPodName, defaultPodServiceAccountName, map[string]string{
// 			OpenFeatureAnnotationPrefix:               "enabled",
// 			"openfeature.dev/featureflagsource": "im-not-real",
// 		})
// 		err := k8sClient.Create(testCtx, pod)
// 		require.Nil(t, err)
// 		defer podMutationWebhookCleanup(t)
...
// 	t.Run("should not create flagd sidecar if featureflagsource  contains a source that does not exist", func(t *testing.T) {
// 		pod := testPod(defaultPodName, defaultPodServiceAccountName, map[string]string{
// 			OpenFeatureAnnotationPrefix:               "enabled",
// 			"openfeature.dev/featureflagsource": fmt.Sprintf("%s/%s", mutatePodNamespace, featureFlagSourceName3),
// 		})
// 		err := k8sClient.Create(testCtx, pod)
// 		require.Nil(t, err)
// 		defer podMutationWebhookCleanup(t)
...
// 		pod := testPod(defaultPodName, defaultPodServiceAccountName, map[string]string{
// 			OpenFeatureAnnotationPrefix: "enabled",
// 			fmt.Sprintf("%s/%s", OpenFeatureAnnotationPrefix, FeatureFlagSourceAnnotation): fmt.Sprintf("%s/%s", mutatePodNamespace, featureFlagSourceName2),
// 		})
// 		err := k8sClient.Create(testCtx, pod)
// 		require.Nil(t, err)
// 		defer podMutationWebhookCleanup(t)
...
// 		pod := testPod(defaultPodName, defaultPodServiceAccountName, map[string]string{
// 			OpenFeatureAnnotationPrefix: "enabled",
// 			fmt.Sprintf("%s/%s", OpenFeatureAnnotationPrefix, FeatureFlagSourceAnnotation): fmt.Sprintf("%s/%s", mutatePodNamespace, flagSourceConfigGrpc),
// 		})
// 		err := k8sClient.Create(testCtx, pod)
// 		require.Nil(t, err)
// 		defer podMutationWebhookCleanup(t)
...
// 	setupValidateFeatureFlagResources(t)
...
// func setupValidateFeatureFlagResources(t *testing.T) {
// 	ns := &corev1.Namespace{}
// 	ns.Name = featureFlagSourceNamespace
...
// 	ffConfig := &v1alpha1.FeatureFlag{}
// 	ffConfig.Namespace = mutatePodNamespace
// 	ffConfig.Name = featureFlagSourceName
...
// 	fsConfig := &v1alpha1.FeatureFlagSource{}
// 	fsConfig.Namespace = mutatePodNamespace
// 	fsConfig.Name = featureFlagSourceName
...
// 	ffConfig2 := &v1alpha1.FeatureFlag{}
// 	ffConfig2.Namespace = mutatePodNamespace
// 	ffConfig2.Name = featureFlagSourceName2
// 	ffConfig2.Spec.FeatureFlagSpec = featureFlagSpec
// 	err = k8sClient.Create(testCtx, ffConfig2)
// 	require.Nil(t, err)

// 	fsConfig2 := &v1alpha1.FeatureFlagSource{}
// 	fsConfig2.Namespace = mutatePodNamespace
// 	fsConfig2.Name = featureFlagSourceName2
// 	fsConfig2.Spec.Sources = []v1alpha1.Source{
// 		{
// 			Source: fmt.Sprintf("%s/%s", mutatePodNamespace, featureFlagSourceName),
// 		},
// 		{
// 			Source:   fmt.Sprintf("%s/%s", mutatePodNamespace, featureFlagSourceName2),
...
// 	fsConfig3 := &v1alpha1.FeatureFlagSource{}
// 	fsConfig3.Namespace = mutatePodNamespace
// 	fsConfig3.Name = featureFlagSourceName3
...
// 	fsConfigGrpc := &v1alpha1.FeatureFlagSource{}
